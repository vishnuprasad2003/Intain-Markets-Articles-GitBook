...
description: >-
  Complete task-based guide for investors to review issuer repayment
  declarations, accept or reject repayment, and burn receivables NFTs on-chain
...
3. For each token, the burn job:
   * Verifies the token's current status in LOAN\_METADATA (Snowflake) — must be **Transferred**
   ...
   * Updates LOAN\_METADATA in Snowflake: sets `TOKENIZATION_STATUS` to **Burned**
...
* After a successful on-chain burn, the off-chain LOAN\_METADATA is updated to reflect **Burned** status
...
* The platform re-reads all LOAN\_METADATA rows for the deal to verify every tokenized loan is now in **Burned** status
* If all tokens are burned, the settlement's NFT status is updated from **Retirement pending** to **Retired**
* An audit event (`settlement.token.burned`) is recorded with the category **CHAIN** and action **burn\_token**
