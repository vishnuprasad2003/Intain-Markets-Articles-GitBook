...
**Activity Audit** - The platform includes a centralized Audit Module accessible via the Activity Audit sidebar item. This module provides a cross-module activity log that consolidates actions from all platform modules into a single chronological view. All modules write to this centralized audit trail via `audit.helper.record()`, ensuring consistent and comprehensive tracking. The audit log supports export capabilities for compliance and reporting needs.

### Asset Sale Blockchain Traceability

**Settlement Events** - Asset sale settlement events are recorded on the blockchain, providing immutable traceability for deal settlements.

**Repayment Events** - Asset sale repayment events are similarly recorded on-chain, ensuring permanent and tamper-proof records of repayment activity.
