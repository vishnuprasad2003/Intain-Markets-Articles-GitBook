...
### Asset Sale Status Vocabulary Rename

**Status Rename for Clarity:**
- What was previously "Closed" is now "Active" (indicating post-settlement active state)
- What was "Repaid" is now "Closed" (indicating final deal completion)
- Better reflects the deal lifecycle progression

**Asset Sale Lifecycle Statuses:**
- Full status progression: Draft → Pending Review → Published → Commit → Invest → Settlement In Progress → Settled → Active → Repayment In Progress → Closed
- Terminal statuses: Cancelled, Defaulted, Inactive

### v2 API Migration

**Core Module Migration:**
- Pools, loans, batches, organizations, users, dataroom, securitization, feedback, dashboard, and audit migrated to /api/v2 endpoints
- Standardized Zod validation, cursor-based pagination, and consistent error handling
- Legacy /api/v1 endpoints are being phased out

### E-Signature Provider Migration

**Provider Change:**
- DocuSign support removed
- Replaced with Adobe Sign and ZohoSign as the supported e-signature providers
- Both integrate via the shared e-signature helper architecture

### Authentication Change

**Microsoft Entra SSO:**
- Microsoft Entra SSO added as the primary authentication method alongside direct login
- SSO refresh token handling and session management updated

### Audit Trail Centralization

**Unified Audit Logging:**
- All modules now write to the centralized audit module via `audit.helper.record()`
- Replaces inconsistent per-module audit logging
