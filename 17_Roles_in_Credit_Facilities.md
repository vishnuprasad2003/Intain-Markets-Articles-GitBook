...
description: Comprehensive guide to all credit facility roles — Borrower, Facility Agent, Lender, Servicer, and Paying Agent — their responsibilities, dashboard views, and stage-specific capabilities
---
...
**Status determines action availability.** Even within a role, actions are only available at the appropriate workflow stage. For example, a Borrower can only edit a term sheet when its status is **Draft** or **CHANGES_REQUESTED** — not when it is under Facility Agent review.
